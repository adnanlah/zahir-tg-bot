...
export const addImages = async (input: AddImagesInput) => {
  try {
    const res = await fetch(`http://${ENV.ZAHIR_INSIGHT_URL}/trpc/imageToInvoice.addImages`, {
      method: 'POST',
...
export const verifyPhoneNumber = async (input: VerifyPhoneNumberInput) => {
  try {
    const res = await fetch(`http://${ENV.ZAHIR_INSIGHT_URL}/trpc/auth.verifyPhoneNumber`, {
      method: 'POST',
      body: JSON.stringify(input),
      headers: {
        'Content-Type': 'application/json',
...
export const checkVerificationCode = async (input: CheckVerificationCodeInput) => {
  try {
    const res = await fetch(`http://${ENV.ZAHIR_INSIGHT_URL}/trpc/auth.checkVerificationCode`, {
      method: 'POST',
      body: JSON.stringify(input),
      headers: {
        'Content-Type': 'application/json',
...
export const checkIfVerified = async (input: CheckIfVerifiedInput) => {
  try {
    const res = await fetch(`http://${ENV.ZAHIR_INSIGHT_URL}/trpc/settings.checkIfVerified`, {
      method: 'POST',
      body: JSON.stringify(input),
      headers: {
        'Content-Type': 'application/json',
    ...
    throw new NetworkError('Check if account is verified failed', res.status, await res.json())
  } catch (err: any) {
...
export const checkPointsRemaining = async (input: CheckPointsRemainingInput) => {
  try {
    const res = await fetch(
    ...
    if (err.name === 'TimeoutError') {
      ...
      console.log(`Server Error while checking points remaining: `, err.message)
    }
    throw err
  }
}
